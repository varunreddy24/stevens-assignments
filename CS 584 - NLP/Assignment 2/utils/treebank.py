...
class StanfordSentiment:
    ...
    def tokens(self):
        ...
        for sentence in self.sentences():
            for w in sentence:
                if True in [char.isdigit() or char in string.punctuation for char in w]:
                    continue
                if len(w.strip()) == 0:
                    continue
    ...
    def sentences(self):
        ...
        texts = sampled_train_df['text'].values.astype(str).tolist()
        for text in tqdm(texts, desc='tokenizing'):
            doc = nlp(text)
            for sent in doc.sents:
                sentences += [[token.text.lower() for token in sent]]
...
#                 sentences += [[w.lower() for w in splitted]]
    ...
    def allSentences(self):
        ...
        allsentences = [[w for w in s
            if w in tokens and (0 >= rejectProb[tokens[w]] or random.random() >= rejectProb[tokens[w]])]
